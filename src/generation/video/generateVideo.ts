...
export const generateVideo = async (
  ...
  if (backgroundCandidates.length === 0) {
    throw new Error("No background image or video found.")
  }
  ...
  if (videoFiles.length === 1) {
    ...
    backgroundInput = path.join(inputDir, videoFiles[0])
    useLoopFlag = true
  } else if (videoFiles.length > 1) {
    // Multiple videos: generate looped background
    backgroundInput = path.join(outputDir, "looped_background.mp4")
    await createLoopedBackground(inputDir, backgroundInput, audioDuration)
  } else if (imageFile) {
    // Fallback to static image
    backgroundInput = path.join(inputDir, imageFile)
    useLoopFlag = true
  } else {
